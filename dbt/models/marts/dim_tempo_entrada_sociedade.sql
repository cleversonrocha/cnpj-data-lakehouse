...
WITH date_spine AS (
    ...
    FROM generate_series((SELECT MIN(data_entrada_sociedade) FROM {{ ref('stg_socios') }} WHERE data_entrada_sociedade IS NOT NULL), 
                         (SELECT MAX(data_entrada_sociedade) FROM {{ ref('stg_socios') }} WHERE data_entrada_sociedade IS NOT NULL),
                         INTERVAL '1 day')
),
