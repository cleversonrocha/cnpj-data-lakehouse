{{ config(    
    post_hook=[        
        export_to_s3(bucket_path='gold/dim', file_name='dim_tempo_inicio_atividade')
    ]
) }}
...
WITH date_spine AS (
    ...
    FROM generate_series((SELECT MIN(data_inicio_atividade) FROM {{ ref('stg_estabelecimentos') }} WHERE data_inicio_atividade IS NOT NULL), 
                         (SELECT MAX(data_inicio_atividade) FROM {{ ref('stg_estabelecimentos') }} WHERE data_inicio_atividade IS NOT NULL),
                         INTERVAL '1 day')
),
