{{ config(    
    post_hook=[        
        export_to_s3(bucket_path='gold/dim', file_name='dim_tempo_situacao_especial')
    ]
) }}
...
WITH date_spine AS (
    ...
    FROM generate_series((SELECT MIN(data_situacao_especial) FROM {{ ref('stg_estabelecimentos') }} WHERE data_situacao_especial IS NOT NULL), 
                         (SELECT MAX(data_situacao_especial) FROM {{ ref('stg_estabelecimentos') }} WHERE data_situacao_especial IS NOT NULL),
                         INTERVAL '1 day')
),
